...
<van-tabs  sticky v-for="(itemObj,inx) in goodsObj" :key="inx"
...
//轮播图
  axios.get(API_LIST.getImgUrls)
    ...
    if (localStorage.userName) {
      ...
      this.$notify(_str.username + "您已经登录");
      this.isLogin = false;
      
      
      this.msg = "用户:" +_str.username;
    }
    //查询所有的栏目及栏目所属的商品
  ...
  methods: {
    ...
    logOutBtn() {
      ...
      this.isLogin = true;
      this.$notify(localStorage.userName + "您退出登录");
    },
    //登录
    loginBtn() {
