...
<van-submit-bar :price=allGoodsMoney*100
...
export default {
  ...
  methods: {
    ...
    ...mapActions(['addToVuex']),
      ...
      onSubmit(){
           this.$dialog.alert({
          message: '共'+this.allGoodsNum+'个商品，共'+this.allGoodsMoney+'元',
        }).then(()=>{
          Toast.success('支付成功');
        })
      },
...
//计算总价
  countAllGoodsMoney(){
            ...
            for(let i=0;i<this.cartArr.length;i++){
                this.allGoodsMoney+=
                this.cartArr[i]._num*this.cartArr[i]._price;
            }
